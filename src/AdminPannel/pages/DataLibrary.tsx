    ...
    { 
      ...
      render: (value: string) => (
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
          value === 'Database' ? 'bg-blue-100 text-blue-800' :
          value === 'Dataset' ? 'bg-green-100 text-green-800' :
          value === 'Reports' ? 'bg-purple-100 text-purple-800' :
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Storage Usage by Type</h3>
          <div className="h-48 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg flex items-center justify-center">
            <div className="text-center">
              <div className="text-3xl mb-2">📊</div>
              <p className="text-gray-600 font-medium">Storage Distribution Chart</p>
              <p className="text-sm text-gray-500">Pie chart showing storage by data type</p>
            </div>
          </div>
        </div>

        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Data Growth Trend</h3>
          <div className="h-48 bg-gradient-to-br from-green-50 to-teal-50 rounded-lg flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500">Data volume growth over time</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h4 className="text-md font-semibold text-gray-900 mb-4">Data Quality Score</h4>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            {[
              { action: 'Data imported', source: 'Customer Database', time: '2 hours ago' },
              { action: 'Schema updated', source: 'Product Catalog', time: '1 day ago' },
              { action: 'Report generated', source: 'Analytics Reports', time: '30 minutes ago' },
              { action: 'Backup completed', source: 'Financial Data', time: '3 hours ago' }
            ].map((update, index) => (
              <div key={index} className="flex items-start space-x-3">
                <div className="w-2 h-2 bg-blue-500 rounded-full mt-2"></div>
                <div className="flex-1">
                  <p className="text-sm font-medium text-gray-900">{update.action}</p>
                  <p className="text-xs text-gray-500">{update.source} • {update.time}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h4 className="text-md font-semibold text-gray-900 mb-4">Data Connections</h4>
          <div className="space-y-4">
            {[
              ...
              <div key={index} className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-sm text-gray-700">{connection.service}</span>
                </div>
                <span className="text-xs text-gray-500">{connection.count} sources</span>
              </div>
            ))}
          </div>
        </div>
