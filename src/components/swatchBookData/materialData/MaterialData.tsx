...
const MaterialData = () => {
    ...
    const { wallMaterials, isWallLoading, isFloorLoading,doorMaterials, roofMaterials, windowMaterials, trimMaterials, floorMaterials } = useSelector((state: RootState) => state.materials);
    ...
    useEffect(() => {
        ...
        if (!wallMaterials || wallMaterials.length === 0 && !isWallLoading) {
            isWall.current = false; // Set the flag to true after the first fetch
            getFetchWallMMaterials()
        }
     if (!floorMaterials || floorMaterials.length === 0 && !isFloorLoading) {
                isFloor.current = false; // Set the flag to true after the first fetch
            getFetchFloorMaterials()
        }
        ...
        // }
    }, [wallMaterials,isWallLoading, isFloorLoading, doorMaterials, roofMaterials, windowMaterials, trimMaterials,floorMaterials]);
    ...
    const getFetchFloorMaterials = async () => {
        try {
           await dispatch(fetchFloorMaterials({ page: 0, limit: 30 })).unwrap();
