...
const LayerSegTab = () => {
  ...
  const DropdownMenu = ({ layerId }: { layerId: number }) => {
    ...
    return (
      <div
        ref={dropdownRef}
        className="absolute right-2 top-14 z-10 w-48 bg-white rounded-md shadow-lg border border-gray-200"
      >
        <ul className="py-1">
          {menuItems.map((item, index) => (
            <li
              ...
              className={`px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer ${
                item === "Delete" ? "text-red-600 hover:bg-red-50" : ""
              }`}
            >
              {item}
            </li>
  ...
  return (
    <div className="w-full  mx-auto border border-gray-200 font-sans">
      <ul className="py-2">
        {masterArray.map((layer, index) => {
          const allSeg = layer.allSegments as MasterGroupModel[];
          const color = layer.color_code;
          console.log("allSeg color", layer.name, color);

          return (
            <React.Fragment key={index}>
              {allSeg &&
                allSeg.length > 0 &&
                allSeg.map((seg: MasterGroupModel, segIndex: number) => {
                  const segAll = seg.segments as SegmentModal[];
                  return (
                    <React.Fragment key={segIndex}>
                      {segAll.map((segment, segmentIndex) => (
                        <li
                          ...
                          className={`flex items-center justify-between px-4 py-3 transition-all duration-200 ease-in-out relative
                                                    ${
                                                      draggedItemIndex ===
                                                      segmentIndex
                                                        ? "opacity-50 bg-blue-100 shadow-lg scale-105"
                        ...
                        >
                          <div
                            ...
                            onMouseEnter={() => {
                              handleMouseEnter(segment.short_title ?? "");
                            }}
                            onMouseLeave={() => {
                              handleMouseLeave();
                            }}
                          >
                            ...
                            <span className="text-gray-800">
                              {segment.short_title || "Unnamed Segment"}
                            </span>
                          </div>
                          <div
                            className="flex items-center gap-3"
                          ...
                          >
                            <DragHandleIcon />
                            <button
                              onClick={() => toggleDropdown(segment.id || 0)}
                              className="focus:outline-none"
                            >
                              <MoreIcon />
                            </button>
                          </div>
                          {openDropdownId === (segment.id || 0) && (
                            <DropdownMenu layerId={segment.id || 0} />
                          )}
                        </li>
                      ))}
                    </React.Fragment>
