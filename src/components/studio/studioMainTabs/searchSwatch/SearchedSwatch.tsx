...
const SearchedSwatch = ({
  ...
  const { filterSwatch, isFetchingSwatch, isFetchingBrand, isFetchingStyle } =
    useSelector((state: RootState) => state.filterSwatch);
  const {
    materials,
    wallMaterials,
    doorMaterials,
  ...
  const searchSwatchByCategoryId = async (categoryId: number) => {
    const { status, data, error } =
      await MaterialService.getMaterialsByCategory_Id(categoryId);
    if (status && data) {
      ...
      isApiCategory.current = true;
      toast.error(
        "Error fetching swatches by category. Please try again later."
      );
    }
  };
  ...
  const searchSwatchByBrandId = async (
    brandId: number[],
    categoryId: number
  ) => {
    const { status, data, error } =
      await MaterialService.getMaterialsByCategoryAndBrand(categoryId, brandId);
  ...
  const searchSwatchByStyleId = async (
    categoryId: number,
    brandId: number[],
    styleId: number[]
  ) => {
    const { status, data, error } =
      await MaterialService.getMaterialsByCategoryBrandAndStyle(
        categoryId,
        brandId,
        styleId
      );
  ...
  // const isPresent:boolean = finaldata?.includes(recommendedSwatches?.id);
  // console.log(recommendedSwatches);
  return (
    <>
      <div className="grid grid-cols-3 gap-3 overflow-y-auto max-h-[50vh] sm:max-h-[50vh]">
        {isFetchingSwatch ? (
          ...
          recommendedSwatches.map((src, idx) => {
            const isPresent = finaldata?.includes(src?.id);
            return (
              <button
                key={idx}
                className={`aspect-square rounded-lg overflow-hidden border border-gray-200 focus:outline-none focus:ring-2 focus:ring-purple-500 p-0 ${
                  isPresent ? "border-2 border-black" : "border-none"
                }`}
                title={`Swatch ${idx + 1}`}
                onClick={() => {
