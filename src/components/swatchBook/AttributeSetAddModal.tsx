    ...
    if (product?.id!) {
      ...
      if (!group_error) {
        ...
        for (const groupItem of group) {
          const matchingGroup = groups_ids.find(
            (g) => g.name === groupItem.name
          );

          if (matchingGroup && Array.isArray(groupItem.attribute_ids)) {
            for (const attributeId of groupItem.attribute_ids) {
              newArray.push({
                attribute_group_id: matchingGroup.id,
