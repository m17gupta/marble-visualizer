...
export const updateJobCommentReply = createAsyncThunk(
    ...
    async ({ jobId, reply }: { jobId: number; reply: string }, { rejectWithValue }) => {
        try {
            const response = await JobCommentService.updateJobCommentReply(jobId, reply);
            if (!response.success) {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(updateJobCommentReply.fulfilled, (state, action) => {
                ...
                action.payload.forEach(updatedComment => {
                    const index = state.projectComments.findIndex(comment => comment.id === updatedComment.id);
                    if (index !== -1) {
