  ...
  static async uploadFile(
    ...
    try {
      ...
      if (file.size > 100 * 1024 * 1024) { // Over 100MB
        console.warn(`Uploading very large file (${Math.round(file.size / (1024 * 1024))}MB). This may cause memory issues in the browser.`);
      }
      ...
      if (file.size <= 100 * 1024 * 1024) { // Under 100MB, still read into memory
        try {
          ...
          console.warn('Failed to convert file to buffer, falling back to streaming upload:', bufferError);
        }
      }
      ...
      if (error instanceof Error && 
        ...
        console.warn('Upload error detected, falling back to single-part upload method:', error.message);
        ...
        try {
          ...
          return await this.uploadSmallFile(s3ClientForFallback, file, fallbackKey, onProgress);
        } catch (fallbackError) {
          console.error('Fallback upload also failed:', fallbackError);
          return {
            success: false,
            error: 'Upload failed even with fallback method. This might be due to file size or network issues. Please try again with a smaller file or check your connection.'
          };
        }
      }
  ...
  private static async uploadSmallFile(
    ...
    try {
      ...
      if (file.size > 100 * 1024 * 1024) { // 100MB
        throw new Error(`File too large for direct upload (${Math.round(file.size / (1024 * 1024))}MB). Maximum recommended size is 100MB.`);
      }
